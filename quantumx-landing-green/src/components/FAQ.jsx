  ...
  return (
    <div className="border-b border-white/10 last:border-0">
      <button 
        ...
        className="w-full text-left py-6 flex items-center justify-between group hover:text-[#10B981] transition-colors"
      >
        <span className="text-xl font-medium pr-8">{q}</span>
        <span className={`text-2xl transform transition-transform duration-300 ${isOpen ? 'rotate-45 text-[#10B981]' : 'rotate-0 text-[#BCD2EE]'}`}>+</span>
      </button>
      <AnimatePresence>
        {isOpen && (
          <motion.div
          ...
          >
            <p className="pb-6 text-[#BCD2EE] leading-relaxed pr-8 border-l-2 border-[#10B981] pl-4 ml-2">
              {a}
            </p>
          </motion.div>
        )}
    ...
    <section className="py-24 px-4 bg-[#010B32]">
      <div className="max-w-4xl mx-auto">
        <h2 className="text-3xl md:text-5xl font-bold text-center mb-16">
          Domande <span className="text-[#10B981]">Frequenti</span>
        </h2>
